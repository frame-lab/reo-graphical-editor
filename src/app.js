...
const codeEditor = monaco.editor.create(document.getElementById("text"), {
  language: "reo",
});
...
async function loadSource(fileName, cb) {
  ...
  client.onreadystatechange = () => {
    if (this.readyState === 4) {
      if (this.status !== 200)
        throw new Error(
          `Error returned with status ${this.status}: ${this.statusText}`
        );
...
function resizeElements() {
  ...
  if (canvas) {
    ...
    let x1 = 25,
      y1 = 25,
      x2 = container.clientWidth - 25,
      y2 = container.clientHeight - 25;
    main.set({
      left: x1,
      top: y1,
      width: x2 - x1,
  ...
  switch (format) {
    ...
    case "treo":
      a.href = window.URL.createObjectURL(
        new Blob([codeEditor.getValue()], { type: "text/plain" })
      );
      break;
    case "nuXmvCompact":
      httpPostAsync(
        "http://127.0.0.1:8081/nuXmv/compact",
        downloadResponse,
        "compactNuXmv.smv"
      );
      break;
    case "nuXmvComponents":
      httpPostAsync(
        "http://127.0.0.1:8081/nuXmv/components",
        downloadResponse,
        "componentsNuXmv.smv"
      );
      break;
    ...
    case "haskellModel":
      httpPostAsync(
        "http://127.0.0.1:8081/haskell/model",
        downloadResponse,
        "haskellModel.hs"
      );
      break;
...
document.getElementById("nuXmvCompact").onclick = async () =>
  download("nuXmvCompact");
document.getElementById("nuXmvComponents").onclick = async () =>
  download("nuXmvComponents");
document.getElementById("coqModel").onclick = async () => download("coqModel");
document.getElementById("haskellModel").onclick = async () =>
  download("haskellModel");
    ...
    if (xmlHttp.readyState == 4) {
      if (this.status !== 200) {
        ...
        if (filename === "haskellModel.hs" && this.status === 569)
          alert(
            "Error in generating Haskell Model: Coq version 8.7 or higher is required."
          );
        throw new Error(
          `Error returned with status ${this.status}: ${this.statusText}`
        );
      }
      if (filename) callback(xmlHttp.response, filename);
...
function createNode(left, top, name, manual) {
  ...
  fabric.Image.fromURL("img/delete.svg", (img) =>
    createAnchor(img, "delete", node.left - 15, node.top + 15)
  );
  fabric.Image.fromURL("img/split.svg", (img) =>
    createAnchor(img, "split", node.left + 15, node.top + 15)
  );
...
function completeChannelCreation(channel, node1, node2, manual) {
  ...
  fabric.Image.fromURL("img/delete.svg", function (img) {
    ...
    const invertedBossTransform = fabric.util.invertTransform(
      channel.parts[0].calcTransformMatrix()
    );
    img.relationship = fabric.util.multiplyTransformMatrices(
  ...
  if (manual) updateNodeColouring(channel.node1);
  else {
  ...
  for (i = 0; i < components.length; ++i) {
    ...
    if (
      Math.abs(p.left - (components[i].left + components[i].width)) <
      mergeDistance
    )
      p.set("left", components[i].left + components[i].width);
    if (
      Math.abs(p.top - (components[i].top + components[i].height)) <
      mergeDistance
    )
  ...
  for (i = 0; i < p.channels.length; ++i) updateChannel(p.channels[i]);
  ...
  for (i = nodes.length - 1; i >= 0; --i) {
    if (nodes[i] === p || nodes[i] === channel.node2) continue;
    if (p.intersectsWithObject(nodes[i]))
      if (
        Math.abs(p.left - nodes[i].left) < mergeDistance &&
        Math.abs(p.top - nodes[i].top) < mergeDistance
      )
        mergeNodes(nodes[i], p);
  }
  fromBoundary = isBoundaryNode(channel.node1);
  canvas.setActiveObject(channel.node2);
  if (manual) isDown = true;
}
...
async function loadChannels() {
  ...
  for (let i = 0; i < channelTypes.length; ++i) loadChannel(channelTypes[i]);
}
...
function setParent(p) {
  ...
  switch (p.class) {
    case "node":
      ...
      for (i = 0; i < p.channels.length; ++i) setParent(p.channels[i]);
      break;
    case "channel":
      if (p.node1.parent.index < p.node2.parent.index)
        p.parent = p.node1.parent;
      else p.parent = p.node2.parent;
...
function updateNode(node, keepParent) {
  ...
  node.set({
    labelOffsetX: node.label.left - node.left,
    labelOffsetY: node.label.top - node.top,
  });
  for (let i = nodes.length - 1; i >= 0; --i) {
    ...
    if (node.intersectsWithObject(nodes[i]))
      if (
        Math.abs(node.left - nodes[i].left) < mergeDistance &&
        Math.abs(node.top - nodes[i].top) < mergeDistance
      )
        mergeNodes(node, nodes[i]);
  }
  if (!keepParent) setParent(node);
...
function updateNodeColouring(node) {
  ...
  for (i = 0; i < node.channels.length; ++i) {
    if (node.channels[i].node1 === node) {
      if (node.channels[i].end1 === "source") source = true;
      else sink = true;
    } else if (node.channels[i].node2 === node) {
      if (node.channels[i].end2 === "source") source = true;
      else sink = true;
    } else
      throw new Error(
        "Internal datastructure has broken down at node " + node.id
      );
  }

  if (source) {
    if (sink) node.set({ nodeType: "mixed", fill: nodeFillColourMixed });
    else node.set({ nodeType: "source", fill: nodeFillColourSource });
  } else node.set({ nodeType: "sink", fill: nodeFillColourSink });
  canvas.requestRenderAll();
}

function updateChannel(channel) {
  const x1 = channel.node1.get("left"),
  ...
  for (let i = 1; i < channel.parts.length; ++i) {
    ...
    if (o.type === "line") o.set({ x1: x1, y1: y1, x2: x2, y2: y2 });
    else {
      ...
      if (o.scale === false) {
        ...
        else o.set({ scaleX: 1, scaleY: 1 });
      }
      if (o.rotate === false) o.set("angle", o.baseAngle);
      let reference;
      switch (o.referencePoint) {
        ...
        case "node2":
          ...
          break;
        default:
          if (channel.parts[0].type === "rect") reference = channel.parts[0];
          else
            reference = {
              left:
                loopRadius *
                  Math.cos(((channel.parts[0].angle - 90) * Math.PI) / 180) +
                channel.parts[0].left,
              top:
                loopRadius *
                  Math.sin(((channel.parts[0].angle - 90) * Math.PI) / 180) +
                channel.parts[0].top,
            };
      }
      o.set({
        left:
          o.referenceDistance *
            Math.cos(
              ((channel.parts[0].angle + o.referenceAngle + 180) * Math.PI) /
                180
            ) +
          reference.left,
        top:
...
function updateText() {
  ...
  codeEditor.setValue(
    components.map((c) => c.toReoDefinition(true)).join("\n")
  );
}

function snapToComponent(node, comp) {
  const right = comp.left + comp.scaleX * comp.width,
    bottom = comp.top + comp.scaleY * comp.height;
  if (node.left > right)
    // right side
    node.set("left", right);
  if (node.left < comp.left)
    // left side
    node.set("left", comp.left);
  if (node.top > bottom)
    // bottom side
    node.set("top", bottom);
  if (node.top < comp.top)
    // top side
    node.set("top", comp.top);
  node.setCoords();
  node.label.set({
    left: node.left + node.labelOffsetX,
    top: node.top + node.labelOffsetY,
  });
  node.label.setCoords();
...
function repositionParts(c) {
  ...
  c.header.set({
    x1: c.left,
    y1: c.top + headerHeight,
    x2: c.left + c.scaleX * c.width,
    y2: c.top + headerHeight,
  });
  c.header.setCoords();
  if (c.delete) {
...
function repositionNodes(c) {
  for (let i = 0; i < c.nodes.length; ++i) {
    let node = c.nodes[i];
    if (node.origLeft === origLeft) node.set("left", c.left);
    if (node.origLeft === origRight)
      node.set("left", c.left + c.scaleX * c.width);
    if (node.origTop === origTop) node.set("top", c.top);
...
canvas.on("selection:created", function (e) {
  if (e.target.delete) e.target.delete.set("visible", true);
  if (e.target.split) e.target.split.set("visible", true);
  canvas.requestRenderAll();
});
...
canvas.on("selection:updated", function (e) {
  let i;
  for (i = 0; i < nodes.length; ++i) {
    if (nodes[i].delete) nodes[i].delete.set("visible", false);
    if (nodes[i].split) nodes[i].split.set("visible", false);
  }
  for (i = 0; i < channels.length; ++i)
...
canvas.on("before:selection:cleared", function (e) {
  ...
  if (e.target.split) e.target.split.set("visible", false);
  canvas.requestRenderAll();
});
...
canvas.on("mouse:down", function (e) {
  ...
  origX = pointer.x;
  origY = pointer.y;
  if (p && mode !== "select") canvas.discardActiveObject(null);
  switch (mode) {
    case "select":
      if (p) {
        switch (p.class) {
          case "node":
            ...
            for (i = 0; i < p.channels.length; ++i) {
              if (p.channels[i].node1 === p) otherNode = "node2";
              else otherNode = "node1";
...
canvas.on("mouse:move", function (e) {
  ...
  switch (p.class) {
    case "component":
      if (p.status === "drawing") {
        if (origX > x) p.set("left", x);
        if (origY > y) p.set("top", y);
        p.set({ width: Math.abs(origX - x), height: Math.abs(origY - y) });
        p.setCoords();
      } else {
        if (p.__corner !== 0) repositionNodes(p);
        else {
          ...
          for (i = 0; i < p.nodes.length; ++i) {
            let node = p.nodes[i];
            node.set({
              left: node.origLeft + x - origX,
              top: node.origTop + y - origY,
            });
            node.setCoords();
            node.label.set({
              left: node.left + node.labelOffsetX,
    ...
    case "node":
      ...
      if (!fromBoundary) {
        ...
        if (p.left < p.parent.left + mergeDistance)
          // near left boundary
          p.set("left", p.parent.left);
        if (p.top < p.parent.top + headerHeight)
          // near top boundary
          p.set("top", p.parent.top);
        if (p.left > p.parent.left + p.parent.width - mergeDistance)
          // near right boundary
          p.set("left", p.parent.left + p.parent.width);
        if (p.top > p.parent.top + p.parent.height - mergeDistance)
      ...
      if (fromBoundary || p.parent === main) {
        for (i = 0; i < components.length; ++i) {
          ...
          if (
            p.left > components[i].left - mergeDistance &&
        ...
        if (
          index >= 0 &&
          components[index] !== p.parent &&
          components[index] !== main
        ) {
          // Ensure that the node is inside the component
          if (p.left < components[index].left)
            // near left boundary
            p.set("left", components[index].left);
          if (p.top < components[index].top)
            // near top boundary
            p.set("top", components[index].top);
          if (p.left > components[index].left + components[index].width)
            // near right boundary
            p.set("left", components[index].left + components[index].width);
          if (p.top > components[index].top + components[index].height)
            // near bottom boundary
          ...
          if (p.top - components[index].top < distance) {
            // top boundary
          ...
          if (
            components[index].left + components[index].width - p.left <
            distance
          ) {
            // right boundary
            distance =
          ...
          if (
            components[index].top + components[index].height - p.top <
            distance
          ) {
            // bottom boundary
            changingPosition = "top";
          ...
          const newPosition = {};
          newPosition[changingPosition] =
            components[index][changingPosition] + size;
          p.set(newPosition);
          p.setCoords();
        }
      }
      ...
      p.label.set({
        left: p.left + p.labelOffsetX,
        top: p.top + p.labelOffsetY,
      });
      if (p.delete)
  ...
  if (p) {
    ...
    switch (p.class) {
      case "node":
        ...
        if (mode === "split") buttonClick(document.getElementById("select"));
        break;
      case "component":
        p.set({
          width: p.scaleX * p.width,
          height: p.scaleY * p.height,
          scaleX: 1,
          scaleY: 1,
        });
        p.setCoords();
        if (p.status === "drawing") p.set("status", "design");
        p.set("selectable", mode === "select");
        bringComponentToFront(p);
        if (mode !== "select") buttonClick(document.getElementById("select"));
        break;
      case "label":
        p.setCoords();
        p.object.set({
          labelOffsetX: p.left - p.object.left,
          labelOffsetY: p.top - p.object.top,
        });
        break;
    }
    if (mode !== "select") canvas.discardActiveObject(null);
    canvas.requestRenderAll();
    updateText();
  }
}
...
function mergeNodes(destination, source) {
  ...
  for (j = 0; j < source.channels.length; ++j) {
    ...
    if (source.channels[j].node1 === source) {
      ...
      if (source.channels[j].node2 === destination) loop = true;
    } else {
      if (source.channels[j].node2 === source) {
        source.channels[j].node2 = destination;
        if (source.channels[j].node1 === destination) loop = true;
      } else throw new Error("channel is not connected to node");
    }
    if (loop) {
      // if the source node is equal to the destination node, create a loop and update the position of all channel parts
      const channel = source.channels[j],
        rect = channel.parts[0],
        line = channel.parts[1];
      ...
      for (i = 2; i < channel.parts.length; ++i) {
        const o = channel.parts[i];
        if (
          o.referencePoint === "node1" ||
          o.referencePoint === "node2" ||
          o.referencePoint === "middle"
        ) {
          // calculate the distance along the straight line
          let length =
            o.referenceDistance *
            Math.cos(((rect.angle + o.referenceAngle + 180) * Math.PI) / 180);
          // calculate the offset from the straight line
          let offset =
            o.referenceDistance *
            Math.sin(((rect.angle + o.referenceAngle + 180) * Math.PI) / 180);
          ...
          if (o.referencePoint === "middle") angleA += 180;
          ...
          o.set({
            left:
              (loopRadius + offset) *
                Math.cos(((angleA + 90) * Math.PI) / 180) +
              curve.left,
            top:
              (loopRadius + offset) *
                Math.sin(((angleA + 90) * Math.PI) / 180) +
              curve.top,
          });
          let diffX, diffY;
      ...
      canvas.remove(line);
    } else destination.channels.push(source.channels[j]);
  }
  for (i = 0; i < nodes.length; ++i)
    if (nodes[i] === source) {
      nodes.splice(i, 1);
...
function prepareSplit(node) {
  ...
  for (j = 0; j < nodes.length; ++j) {
    if (nodes[j].selection.visible === true) {
      ...
      for (i = 0; i < nodes[j].channels.length; ++i)
        nodes[j].channels[i].parts[0].set({
          fill: "transparent",
          selectable: false,
          hoverCursor: "default",
        });
  ...
  for (i = 0; i < node.channels.length; ++i)
    node.channels[i].parts[0].set({
      fill: splitDeselected,
      selectable: true,
      hoverCursor: "pointer",
    });
  canvas.requestRenderAll();
...
function splitNode(source) {
  ...
  for (i = 0; i < destination.channels.length; ++i) {
    if (destination.channels[i].node1 === destination) otherNode = "node2";
    else otherNode = "node1";
...
function bringComponentToFront(p) {
  ...
  for (; i < limit; ++i) components[i].set("index", i);
  p.bringToFront();
  p.header.bringToFront();
  if (p.delete) p.delete.bringToFront();
  if (p.compactSwitch) p.compactSwitch.bringToFront();
  if (p.copy) p.copy.bringToFront();
  p.label.bringToFront();
  // Set a new parent for the channels if necessary
  for (i = p.channels.length - 1; i >= 0; --i) setParent(p.channels[i]);
  for (i = 0; i < p.channels.length; ++i)
...
function bringNodeToFront(p) {
  ...
  p.label.bringToFront();
  if (p.delete) p.delete.bringToFront();
  if (p.split) p.split.bringToFront();
}
...
function deleteChannel(channel) {
  ...
  if (channel.node1)
    for (j = 0; j < channel.node1.channels.length; ++j)
      if (channel.node1.channels[j] === channel) {
        ...
        if (channel.node1.channels.length === 0) deleteNode(channel.node1);
        else updateNodeColouring(channel.node1);
        break;
      }
  if (channel.node2)
    for (j = 0; j < channel.node2.channels.length; ++j)
      if (channel.node2.channels[j] === channel) {
        ...
        if (channel.node2.channels.length === 0) deleteNode(channel.node2);
        else updateNodeColouring(channel.node2);
        break;
      }
  ...
  for (j = 0; j < channel.parts.length; ++j) canvas.remove(channel.parts[j]);
}
...
function deleteComponent(component, recursive) {
  ...
  for (k = 0; k < component.nodes.length; ++k) deleteNode(component.nodes[k]);
...
function copyComponent(p) {
  ...
  for (i = 0; i < p.channels.length; ++i) {
    ...
    createChannel(
      c.name,
      { x: c.node1.left + 20, y: c.node1.top + 20, name: c.node1.id },
      {
        x: c.node2.left + 20,
        y: c.node2.top + 20,
        name: c.node2.id,
...
document.addEventListener("keydown", function (e) {
  ...
  switch (e.code) {
    ...
    case "KeyZ":
      if (e.ctrlKey) {
        if (e.shiftKey) console.log("Ctrl + Shift + Z");
        // TODO redo
        else console.log("Ctrl + Z"); // TODO undo
      }
  }
});

function createComponent(x1, y1, x2, y2, name, manual) {
  // FIXME parent component must be set for code generation
  const width = x2 - x1,
    height = y2 - y1,
    left = x1,
    top = y1;
  ...
  const header = new fabric.Line(
    [x1, y1 + headerHeight, x2, y1 + headerHeight],
    {
      fill: "#000",
      stroke: "#000",
      strokeWidth: 1,
  ...
  if (name !== "main") {
    ...
    fabric.Image.fromURL("img/delete.svg", (img) =>
      createAnchor(img, "delete")
    );
    fabric.Image.fromURL("img/compact.svg", (img) =>
      createAnchor(img, "compactSwitch")
    );
  ...
  component.toReoDefinition = function (withComment) {
    return (
      ...
      (this.channels.length > 0
        ? "\t//Channels\n" +
          this.channels
            .map((c) => `\t${c.toReo ? c.toReo(withComment) : ""}`)
            .join("") +
          "\n"
        : "") +
  ...
  component.toReoInstance = function (withComment) {
    return `${this.id}(${this.nodes
      .filter(isBoundaryNode)
      .map((n) => n.id)
      .join(", ")})${withComment ? this.generatePositionMetadata() : ""}\n`;
  };
